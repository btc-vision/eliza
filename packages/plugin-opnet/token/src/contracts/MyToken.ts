...
export class MyToken extends DeployableOP_20 {
    ...
    public override onDeployment(_calldata: Calldata): void {
        const maxSupply: u256 = u256.fromString('{{SUPPLY}}'); // Your max supply.
        const decimals: u8 = {{DECIMALS}}; // Your decimals.
        const name: string = '{{NAME}}'; // Your token name.
        const symbol: string = '{{SYMBOL}}'; // Your token symbol.

        this.instantiate(new OP20InitParameters(maxSupply, decimals, name, symbol));

        // Add your logic here. Eg, minting the initial supply:
        this._mint(new Address([{{MINT_ADDRESS}}]), maxSupply);
    }
